module Main where
    ...
    puzzle1 :: String -> Int
    puzzle1 s = let m = parseInput s in (sum . map (length . nub . explore m)) (startingPositions m) 

    puzzle2 :: String -> Int
    puzzle2 s = let m = parseInput s in (sum . map (length . explore m)) (startingPositions m)
